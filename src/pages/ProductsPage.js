...
import ProductsGrid from "../partials/products-collection/product-grid";
...
function ProductsPage() {
  ...
  const [sortBy, setSortBy] = useState(
    query?.sortBy ? query.sortBy : "default"
  );
  const products = data && data.products;
  const totalPage = data
    ? parseInt(data.products.length / perPage) +
      (data.products.length % perPage ? 1 : 0)
    : 1;
...
// export default withApollo({ ssr: typeof window === "undefined" })(ProductsPage);
